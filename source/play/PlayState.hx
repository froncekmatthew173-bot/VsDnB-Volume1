...
class PlayState extends MusicBeatState {
	 ...
	 */
	public var onPreferenceChangedPost(default,
		null):FlxTypedSignal<(preference:String, value:Any) -> Void> = new FlxTypedSignal<(preference:String, value:Any) -> Void>();
	...
	override function beatHit(beat:Int):Bool {
		...
		for (i in [camGameZoom, camHUDZoom]) {
			if (i.canZoom
				&& Std.int(curBeat - Std.int(Math.round(Conductor.instance.currentTimeChange.beatTime))) % i.timeSnap == 0
				&& !i.useSteps
				&& camZooming) {
				i.camera.zoom += i.zoomValue;
			}
	...
	function initCharacters():Void {
		...
		var customVariation:Bool = (this.currentVariation == Song.validateVariation(params.targetVariation)
			&& this.currentVariation != Song.DEFAULT_VARIATION);

		var customChar:Null<String> = (PlayStatePlaylist.isStoryMode
			|| FreeplayState.skipSelect.contains(currentSong.id.toLowerCase())
			|| customVariation) ? null : CharacterSelect.selectedCharacter;
	...
	function createStrums():Void {
		dadStrums = new Strumline({
			isPlayer: false,
			noteStyle: dad.skins.get('noteSkin'),
		...
		playerStrums = new Strumline({
			isPlayer: true,
			noteStyle: boyfriend.skins.get('noteSkin'),
			scrollType: scrollType,
			showStrums: false
		});
